...
class CosmosDiTNetwork(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    apply_rope_before_kvcache=self.config.apply_rope_before_kvcache,
                )
    ...
    def initialize_cache(
        ...
        Args:
            chunk_size: Tokens generated by one autoregressive step.
            window_size: Rolling-window capacity used by full-history blocks.
            sink_size: Permanently retained prefix capacity.
            context: Text embeddings cached by cross-attention.
            early_short_history_block_count: Number of initial blocks whose
                rolling window is limited to ``chunk_size``. ``None`` uses
                ``window_size`` for every block.

        Returns:
            Independent self- and cross-attention caches for every block.
        """
        if self.config.use_crossattn_projection:
            context = self.crossattn_proj(context)

        block_caches: list[BlockCache] = []
        short_history_count = early_short_history_block_count or 0
